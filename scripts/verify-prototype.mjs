...
const wdl=fs.existsSync('output/prototype_smoke_wdl.json')?JSON.parse(fs.readFileSync('output/prototype_smoke_wdl.json','utf8')):null;
const checks={policyLoaded:!!policy,split2Coverage:policy?Object.keys(policy.split2).length===12168:false,fillCoverage:policy?Object.keys(policy.fill).length>0:false,build:fs.existsSync('dist/index.html'),smoke:smoke?.smoke_matches===100&&wdl?.matches===100,fuzz:smoke?.fuzz_sequences>=30,report:fs.existsSync('output/prototype_report.md')};
