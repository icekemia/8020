...
const pick = <T>(items: readonly T[], rng: RandomSource): T =>
  sample(
    items.map((item) => [item, 1 / items.length]),
    rng,
  );
export function fillPayoff(
  ...
  return Math.sign(
    margins.reduce(
      (score, margin, i) => score + Math.sign(margin + own[i] - opponent[i]),
      0,
    ),
  );
}
function beginnerFill(margins: number[], rng: RandomSource): Fill {
  ...
  const scored = usefulFills(margins).map((action) => ({
    action,
    score:
      opponents.reduce(
        (s, opponent) => s + fillPayoff(margins, action, opponent),
        0,
      ) / opponents.length,
  }));
  const best = Math.max(...scored.map((x) => x.score));
  return pick(
    scored.filter((x) => x.score >= best - 0.25).map((x) => x.action),
    rng,
  );
}
export function usefulFills(margins: readonly number[]): Fill[] {
  const candidates = fills.filter((action) =>
    action.every(
      (n, i) =>
        n <= (Math.abs(margins[i]) > 20 ? 0 : Math.min(20, 21 - margins[i])),
    ),
  );
  // Already decided positions can have no useful allocation; normal play skips their Fill.
  return candidates.length ? candidates : fills;
}
function variedHardFill(
  policy: Policy,
  margins: number[],
  seat: Player,
  rng: RandomSource,
  ...
  const outcomes = fills.map((opponent) =>
    fillPayoff(ownMargins, selected, opponent),
  );
  // Identical payoffs against every legal reply preserve the mixed strategy guarantee.
  const candidates = usefulFills(ownMargins);
  const equivalent = candidates.filter((action) =>
    fills.every(
      (opponent, i) => fillPayoff(ownMargins, action, opponent) === outcomes[i],
    ),
  );
  return pick(
    equivalent.length
      ...
      : candidates.filter((action) =>
          fills.every(
            (opponent, i) =>
              fillPayoff(ownMargins, action, opponent) >= outcomes[i],
          ),
        ),
    rng,
  );
...
export function chooseBotAction(
  ...
  const expert =
    difficulty === "hard" || (difficulty === "medium" && rng.next() < 0.65);
